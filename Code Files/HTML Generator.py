...
for row in range(len(df)):
            ...
            if Label_check == -1:
                ...
                for element in range(len(Labels)):
                    ...
                    if str(Labels.loc[element, 'Boxname']) == str(df.loc[row, 'Name']):
                        ...
                        test_String = str(Labels.loc[element, 'Labelname'])
                        alphabets = list(string.ascii_uppercase) + list(string.ascii_lowercase) 
                        start = 0

                        for i in range(len(test_String)):
                            if test_String[i] in alphabets:
                                start = i
                                break
        
                        test_String = test_String[start:]
                        # Storing the HTML script 
                        
                        # Storing the HTML script 
                        text = '<td>'+'<p>' + str(test_String) + ' <input id="' + name_block2 + '" name="' + name_block + '" value="[*' + label + '*]"' + ' size="' + '"100"' + '"' + ' type="text" />' + '</p>' +'</td>'+' \n'
            ...
            if Label_check == -1:
                ...
                for element in range(len(Labels)):
                    ...
                    if str(Labels.loc[element, 'Boxname']) == str(df.loc[row, 'Name']):
                        ...
                        test_String = test_String[start:]
                        # Storing the HTML script 
                        text = '<td>'+'<p>' + test_String + ' <input id="' + name_block2 + '" name="' + name_block + '" value="[*' + label + '*]"' + ' size="' + '"100"' + '"' + ' type="text" />' + '</p>' +'</td>'+' \n' 
                    else:
                        # Storing the HTML script 
                        text = '<td>'+'<p>' + str(df.loc[row, 'Name']) + ' <input id="' + name_block2 + '" name="' + name_block + '" value="[*' + label + '*]"' + ' size="' + '"100"' + '"' + ' type="text" />' + '</p>' +'</td>'+' \n'
            else:
    ...
    if df.loc[row, "ObjectType"] == "checkbox":
        ...
        if Row_Elements < Threshold:
            ...
            if text_String != "":
                # Storing the HTML script 
                text = '<td>'+'<p>' + test_String + ' <input id="' + name_block2 + '" name="' + name_block + '" value="yes"' + ' type="checkbox" />' + '</p>' +'</td>'+' \n'
            else:
                # Storing the HTML script 
